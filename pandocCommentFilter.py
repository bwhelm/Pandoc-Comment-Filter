...
MARGIN_STYLE = 'max-width:20%; border: 1px solid black; padding: 1ex; margin: 1ex; float:right; font-size: small;' # HTML style for margin notes
...
def handle_comments(key, value, docFormat, meta):
	global INLINE_TAG_STACK, BLOCK_COMMENT, INLINE_COMMENT, INLINE_MARGIN, INLINE_HIGHLIGHT, INLINE_FONT_COLOR_STACK
	
	# If translating to markdown, leave everything alone.
	if docFormat == 'markdown': return
	...
	if key == 'RawBlock':
		...
		if elementFormat != 'html': return
		tag = tag.lower()
		
		if not draft:
			if BLOCK_COMMENT: # Need to suppress output
				if tag == '</!comment>': BLOCK_COMMENT = False
				return []
		...
		if tag in ['<!comment>', '<!box>', '<center>', '<!speaker>']:
			...
			if docFormat == 'latex': return Para([latex(LATEX_TEXT[tag])]) # FIXME: What about beamer?
			elif docFormat in ['html', 'html5']: return Plain([html(HTML_TEXT[tag])])
			elif docFormat == 'revealjs': return Plain([html(REVEALJS_TEXT[tag])])
			else: return
	...
	if not draft and BLOCK_COMMENT: return [] # Need to suppress output
	...
	# Then check to see if we're changing INLINE_TAG_STACK...
		...
		if not draft: # Check to see if need to suppress output. We do this only for `<comment>` and `<margin>` tags; with `<fixme>` and `<highlight>` tags, we merely suppress the tag.
			if tag == '<comment>': 
				...
				if tag == '</comment>': INLINE_COMMENT = False
				return []
			elif INLINE_MARGIN: # Need to suppress output
				if tag == '</margin>': INLINE_MARGIN = False
				return []
		...
		if tag in ['<comment>', '<fixme>', '<margin>', '<highlight>', '</comment>', '</fixme>', '</margin>', '</highlight>']:
			if docFormat in ['latex', 'beamer']: # LaTeX gets treated differently than HTML
				preText = ''
				postText = ''
				if INLINE_HIGHLIGHT and tag != '</highlight>': # Cannot change COLORS within highlighting in LaTeX (but don't do anything when closing the highlight tag!)
				...
				if tag in ['<comment>', '<fixme>', '<margin>', '<highlight>']: # If any opening tag
					if tag == '<comment>': 
						...
						INLINE_FONT_COLOR_STACK.append(INLINE_FONT_COLOR_STACK[-1])
					INLINE_TAG_STACK.append(tag)
					return latex(preText + LATEX_TEXT[tag] + postText)
				elif tag in ['</comment>', '</fixme>', '</margin>', '</highlight>']:
					if tag == '</comment>': INLINE_COMMENT = False
					elif tag == '</fixme>': pass
					elif tag == '</margin>': INLINE_MARGIN = False
					elif tag == '</highlight>': INLINE_HIGHLIGHT = False
					INLINE_FONT_COLOR_STACK.pop()
					...
					if currentInlineStatus[1:] == tag[2:]: # We have a matching opening tag
						return latex('{}{}\\color{{{}}}{{}}{}'.format(preText, LATEX_TEXT[tag], previousColor, postText))
					else: 
				...
				if tag in ['<comment>', '<fixme>', '<margin>', '<highlight>']:
					if tag == '<highlight>': INLINE_HIGHLIGHT = True
					INLINE_TAG_STACK.append(tag)
				else: 
					if tag == '</highlight>': INLINE_HIGHLIGHT = False
					INLINE_TAG_STACK.pop()
				if docFormat in ['html', 'html5']: return html(HTML_TEXT[tag])
				elif docFormat == 'revealjs': return html(REVEALJS_TEXT[tag])
				else: return []
		
		elif tag.startswith('<i ') and tag.endswith('>'): # Index
			if docFormat == 'latex': return latex('\\index{{{}}}'.format(tag[3:-1]))
			else: return []
			
		elif tag.startswith('<l ') and tag.endswith('>'): # My definition of a label
			if docFormat == 'latex': return latex('\\label{{{}}}'.format(tag[3:-1]))
			elif docFormat in ['html', 'html5']: return html('<a name="{}"></a>'.format(tag[3:-1]))
			
		elif tag.startswith('<r ') and tag.endswith('>'): # My definition of a reference
			if docFormat == 'latex': return latex('\\cref{{{}}}'.format(tag[3:-1]))
			elif docFormat in ['html', 'html5']: return html('<a href="#{}">here</a>'.format(tag[3:-1]))
			
		elif tag.startswith('<rp ') and tag.endswith('>'): # My definition of a page reference
			if docFormat == 'latex': return latex('\\cpageref{{{}}}'.format(tag[4:-1]))
			elif docFormat in ['html', 'html5']: return html('<a href="#{}">here</a>'.format(tag[4:-1]))
	
	elif not draft and (INLINE_COMMENT or INLINE_MARGIN): return [] # Suppress all output
		...
		try:
			...
			if value[0]['t'] == 'Str' and value[0]['c'] == '<' and value[1]['t'] == 'Space': 
				if docFormat == 'latex':
					...
					return Para([html('<div class="noindent">')] + value[2:] + [html('</div>')])
				else: return Para(value[2:])
				
			else: return # Normal paragraph, not affected by this filter
	...
	# Check for tikz CodeBlock. If it exists, try typesetting figure
		...
		if 'tikz' in classes or '\\begin{tikzpicture}' in code:
			...
			filetype = '.pdf' if docFormat == 'latex' else '.png' # (with '.' in extension)
			...
			if not path.isfile(sourceFile):
				...
				debug('Created image {}\n\n'.format(sourceFile))
			if caption: # Need to run this through pandoc to get JSON representation so that captions can be docFormatted text.
...
if __name__ == "__main__": toJSONFilter(handle_comments)
